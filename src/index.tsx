...
const CMDK: FC<CMDKProps> = ({ options }) => {
  ...
  const renderOptions = (opts: CMDKOption[]): ReactNode => {
    return filterOptions(opts).map((option) => {
      ...
      return option.options ? (
        <div key={option.key} tw={"p-2 pt-0"}>
          <li tw={"px-2 rounded-md flex items-center h-10"}>
            <span tw={"text-sm font-medium text-gray-500 leading-none"}>
              {option.label}
            </span>
          </li>
        ...
        <li key={option.key}>
          ...
          >
            <div tw={"space-x-3 flex items-center"}>
              {Icon && (
                <div tw={"w-5 flex items-center justify-center"}>
                  <Icon tw={"w-4 text-gray-500"} />
                </div>
              )}

              <span tw={"leading-none"}>{option.label}</span>
            </div>
  ...
  return (
    ...
    >
      <Dialog.Overlay
        tw={`dialog-overlay ${"h-screen w-screen top-0 left-0 bg-black fixed bg-opacity-25 dark:bg-opacity-50"}`}
      />
      <Dialog.Content
        tw={`dialog-content ${"text-gray-900 antialiased dark:text-white fixed top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/2"}`}
      >
        <div
          tw={
        ...
        >
          ...
          {options?.length ? (
            ...
            <div
              tw={"flex-1 flex flex-col space-y-1 items-center justify-center"}
            >
              <HeroIcons.CloudIcon
                tw={"w-20 text-gray-300 dark:text-gray-600"}
              />
              <p>Nothing here...</p>
            </div>
          )}
